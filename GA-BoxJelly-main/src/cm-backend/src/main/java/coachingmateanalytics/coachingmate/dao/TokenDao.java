...
public class TokenDao {
    ...
    public void deleteAccessToken(String username) {
        ...
        Update update = new Update().unset("userAccessToken").unset("userAccessSecret");
        mongoTemplate.updateMulti(query, update, UserPartner.class, Consts.MONGODB_USER_COLLECTIN_NAME);
    }

    public String findAccessTokenByToken(RequestToken token) {
        Query query = Query.query(Criteria.where("userAccessToken").is(token.getToken()));
        return mongoTemplate.findOne(query, UserPartner.class, Consts.MONGODB_USER_COLLECTIN_NAME).toString();
    }
