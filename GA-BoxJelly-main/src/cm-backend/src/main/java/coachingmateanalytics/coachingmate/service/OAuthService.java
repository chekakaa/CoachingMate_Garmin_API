...
public class OAuthService {
    ...
    public void revokeAccessToken(String accessToken) {
        RequestToken token = tokenDao.findByToken(accessToken); // find access token by token
        if (token != null) { // if token exists
            tokenDao.deleteAccessToken(token.getUsername()); // delete access token by username
        } else {
